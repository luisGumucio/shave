...
//    select m.name, m.id, d.item_item_id from transaction_detail d join ware_house_mp m on d.ware_house_id = m.id group by m.id, m.name, d.item_item_id
    ...
    @Query(value = "SELECT new com.manaco.org.common.dto.TransactionDetailDto(m.id, m.name, d.item.itemId) FROM TransactionDetail d join WareHouseMp m on d.wareHouseId = m.id GROUP BY m.id, m.name, d.item.itemId" )
